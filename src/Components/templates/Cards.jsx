...
function Cards({ data , title }) {
  ...
  return (
    <div className="w-full  flex flex-wrap gap-5 sm:gap-10 justify-center  bg-black  ">
      {data.map((c, i) => (
        <div className="relative  md:flex ">
          <Link to={`/${c.media_type || title }/details/${c.id }`} className="w-[25vh]" key={i}>    
            ...
            <div className="flex justify-center items-center w-[25vh]">
              ...
              {c.vote_average && (<div className="absolute -right-3 bottom-7 sm:text-lg font-semibold text-white w-7 h-7 sm:w-10 sm:h-10 bg-[#FF005C] flex justify-center items-center rounded-full ">{(c.vote_average * 10).toFixed()}<sup>%</sup></div>)}
            </div>
          </Link>
        </div>
      ))}
